...
def get_posts(db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = "", current_user: schemas.User = Depends(oauth2.get_current_user)):
...
def update_post(id: int, post_req: schemas.PostCreate, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
    post_query = db.query(models.Post).filter(models.Post.id == id)
    ...
    post_query.update(post_req.dict(), synchronize_session=False)
    db.commit()
    return post